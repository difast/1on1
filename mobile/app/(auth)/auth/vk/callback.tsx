...
/*
 * Возврат из VK ID в приложение по deep-link:
 *   oneonone://auth/vk/callback?token=...&status=...
 *
 * Обмен кода на токен и выдача JWT произошли на бэкенде (веб-мост
 * /auth/vk/callback?platform=mobile передал code/device_id на сервер, а тот
 * перебросил сюда готовый токен). Здесь остаётся только сохранить сессию:
 * профиль подтягивается с /auth/me тем же путём, что при обычном входе.
 */
export default function VkAuthCallback() {
  ...
  useEffect(() => {
    ...
    if (providerError) { done.current = true; setError(t('auth.vkFailed')); return; }
    ...
    (async () => {
      try {
        ...
        setError(detail?.message || (typeof detail === 'string' ? detail : t('auth.vkFailed')));
      }
    })();
  }, [token, providerError]);
