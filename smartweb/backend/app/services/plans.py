...
Notes on wording (must match /pricing and the in-app plan screen):
  ...
  - Бесплатный старт = тариф Free с его ЗАЯВЛЕННЫМ (ограниченным) набором
    функций — только то, что указано для Free на странице тарифов лендинга
    (базовые: планирование встреч, заметки, трекинг задач; фичи-флаги = False).
    Полного набора функций Free НЕ даёт. Регистрация создаёт 14-дневное Free-окно
    (subscriptions.start_signup_free); по истечении, при ENTITLEMENTS_ENFORCE,
    доступ закрывается до выбора тарифа (LOCKED_LIMITS).
"""
