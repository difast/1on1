...
def detect_region(request) -> str | None:
    ...
    ЗАГЛУШКА: пока в продукте нет отображения цены по региону, определение
    выключено и всегда возвращает None. Иначе при каждом первом входе
    резолвился IP пользователя — в том числе через сторонний сервис — ради
    значения, которое нигде не используется. Снять заглушку следует вместе с
    появлением региональных цен: REGION_PRICING_ENABLED=1."""
