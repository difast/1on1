...
def require_admin(
    ...
    Пока ADMIN_API_TOKEN не задан в окружении и токен пользователя не приходит,
    гвард остаётся совместимым с текущим прод-поведением (админ-панель гейтится
    на клиенте). Жёсткая блокировка включается заданием ADMIN_API_TOKEN и
    переводом админ-интерфейса на присылку токена (Этап 8).
    """
