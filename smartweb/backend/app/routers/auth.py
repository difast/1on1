...
def login(data: LoginReq, background_tasks: BackgroundTasks, request: Request,
    ...
    # Код по email при входе: после верного пароля требуется код из письма.
    # Активен, если включён явным флагом ИЛИ если настроена доставка почты (иначе
    # код нельзя доставить и требовать его нельзя — иначе вход заблокируется). Так
    # поведение работает в проде «из коробки», без ручной установки переменной.
    # ВАЖНО: при включённой 2FA код по email НЕ отправляется — вторым фактором уже
    # служит одноразовый код из приложения-аутентификатора, дублировать его кодом
    # из письма не нужно (о чём пользователь предупреждён при включении 2FA).
    login_code_active = (settings.login_email_code or mailer.configured()) and not user.totp_enabled
